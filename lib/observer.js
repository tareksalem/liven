...
class Observer {
    ...
    async updateIfStatments(path, component) {
        try {
            ...
            elements.forEach((element) => {
                LivenHelpers.ifStatmentResolver(element.updated, element.shadowElement, component);
            })
    ...
    async update(target, propName) {
        ...
        let xpathElements = LivenHelpers.getElementsByXPath(xpaths, self.component.preCompiledElement).length > 0 ? LivenHelpers.getElementsByXPath(xpaths, self.component.preCompiledElement) : LivenHelpers.getElementsByXPath(xpaths, compiledElement);
        if (xpathElements) {
    ...
    async observeArray() {
        ...
        this.watcherArrays.forEach(async(path) => {
            ...
            observedArr.addEventListener("itemadded", async(e) => {
                ...
                let elements = LivenHelpers.getElementsByXPath(xpaths, self.component.preCompiledElement)
                    ...
                    if (matchedElement) {
                        ...
                        try {
                            Array.from(childElements).forEach(async(childElement) => {
                                childElement.setAttribute("element-id", LivenHelpers.generateId())
