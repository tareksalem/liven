...
const LivenHelpers = {};
LivenHelpers.component = {};
LivenHelpers._events = new EventEmitter();

// function to inherent elements to the class
LivenHelpers.component.inherentElements = (self, componentElement) => {
    ...
    if (self.props.style) {
        ...
        Object.keys(style).forEach(function(el) {
            if (el == 'component') {
                ...
                let matchElements = componentElement ? componentElement.querySelectorAll(el): [];
                let cssElements = [];
        ...
        if (typeof methods == "object") {
            ...
            Array.from(Object.keys(methods)).forEach(function(el) {
                if (el == 'component') {
                    ...
                    var elements = componentElement ? componentElement.querySelectorAll(el): [];
                    var matchedElements = [];
                    if (Array.from(elements).length < 1) {
                        let allElements = componentElement ? componentElement.querySelectorAll('[elementName]'): [];
                        if (allElements) {
...
LivenHelpers.validation = {};
LivenHelpers.validation.notEmpty = function(element, cb) {
...
LivenHelpers.validation.strip_html_tags = function(str) {
    if ((str === null) || (str === ''))
        return false;
    else
        str = str.toString();
...
LivenHelpers.validation.checkEmail = function(element, cb) {
        var elementVal = typeof element == "object" ? element.value : typeof element == "string" ? element : "";
...
LivenHelpers.validation.checkIsNumber = function(element, cb) {
    var elementVal = typeof element == "object" ? element.value : typeof element == "string" ? element : "";
    if (elementVal !== "") {
...
LivenHelpers.validation.checkContainsNumber = function(element, count, cb) {
    var elementVal = typeof element == "object" ? element.value : typeof element == "string" ? element : "";
...
LivenHelpers.component.livenDom = (self) => {
    ...
    // file upload method
LivenHelpers.fileUpload = (fileInput, dataInput) => {
...
LivenHelpers.filterContent = function(element, options) {

};

// loop elements method
LivenHelpers.loopElements = function(data, cb) {
...
LivenHelpers.parseData = function(data, cb) {
    if (typeof data === 'string') {
        data = JSON.parse(data);
    }
    if (Array.isArray(data)) {
...
LivenHelpers.getAllMethods = function(obj, deep = Infinity) {
    let props = []
...
LivenHelpers.getElementsByXPath = function(xpaths, parent) {
        let results = [];
...
LivenHelpers.resolveElementId = (component) => {
...
LivenHelpers.ifStatmentResolver = async (updated, shadowElement, self) => {
            ...
            if (shadowElement.getAttribute("A-if")) {
                ...
                if (statment) {
                    ...
                    Array.from(updated.querySelectorAll("*")).forEach((element) => {
                        let foundHistoryElements = self.elementHistory.filter(obj => obj.shadowElement.getAttribute("element-id") == element.getAttribute("element-id"));
                        let shadowElement = element.cloneNode(true);
                        let clonedElement = element.cloneNode(true);
                        // LivenHelpers.removeAttributes(element);
                        foundHistoryElements.forEach(obj => {
                            obj.updated = element;
                            obj.shadowElement = shadowElement
                        })
                        // LivenHelpers.removeAttributes(element)
                    });
                    if (self.methods) {
                        LivenHelpers.component.methods (self, self.self);
                      }
                      LivenHelpers.inlineDomEvents.bind (self) ();
                      await self.getDom ();
                } else {
                    return false;
                }
...
LivenHelpers.inlineDomEvents = function() {
        if (this) {
...
LivenHelpers.elementAttributes = (elem) =>
...
LivenHelpers.elementDataAttributes = (elem) => 
    Object.keys(elem.dataset).reduce((attrMap, name) => {
        attrMap[name] = elem.dataset[name];
        try {
            attrMap[name] = JSON.parse(attrMap[name]);
        } catch (e) {
            attrMap[name] = elem.dataset[name];
...
LivenHelpers.logger = (props) => {
        if (Liven.mode == "development") {
            // console.log(props);
        }
    }
    // method to set deep properties to the object
LivenHelpers.deep_set = function(o, p, newVal) {
    p.split(".").reduce((a, v) => {
        return a[v] = newVal;
    }, o)
}
...
LivenHelpers.deep_value = function(o, p) {
    // console.log(o, p);
    try {
...
LivenHelpers.isNumeric = function(value) {
    return /^-{0,1}\d+$/.test(value);
}
LivenHelpers.byString = function (object, string, value) {
    var _object = object;
