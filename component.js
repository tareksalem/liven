...
class Component {
    constructor(props) {
        ...
        this.id = ActiveHelpers.generateId();
        this.uniqueName = this.props && this.props.uniqueName ? this.props.uniqueName : ActiveHelpers.generateRandomName();
        componentObject.active.components.push(this);
    }
    ...
    async init(cb) {
        ...
        if (methodsArr.length > 0) {
            ...
            Promise.all(methodsArr).then(() => {
                ...
                ActiveHelpers.inlineDomEvents.bind(this)()
            }).catch((e) => {
                ActiveHelpers.logger({
                    type: "error",
                    error: e
