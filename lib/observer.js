...
class Observer {
  ...
  constructor (object, component) {
    ...
    this.target = onChange (this.target, (path, newVal, oldVal) => {
      if (path) {
        ...
        this.renderConditions(path, this.component)
      }
    });
  }
  getWatchArrays () {
  ...
  // method to render conditions
  renderConditions(path, component) {
    try {
      let elements = component.elementHistory.filter (
        obj => {
          return obj.shadowElement.getAttribute (':key') == path
        });
        let deebVal  = LivenHelpers.deep_value(this.target, path);
      elements.forEach (element => {
        let context =element.shadowElement.getAttribute(":context");
        ...
        // if(key && context) {
        //   let keyData = LivenHelpers.deep_value(this.target, key);
        //   data[[context]] = keyData;
        // }
        if(context) {
          context = context.replace(/\s/gi, "").replace(/[\r\n\t]/g, "");
          // context = context.replace(/['"]+/g, '');
          // context = `{${context}}`;
          // context = context.replace(/(['"])?([a-zA-Z0-9_]+)(['"])?:/g, '"$2$3$1": ');
          // context = context.replace(/([{,])(\s*)([A-Za-z0-9_\-]+?)\s*:/g, '$1"$3":')
          // context = JSON.stringify(context);
          // context = JSON.parse(context);
          // var jsonStr2 = '{"' + context.replace(/ /g, '", "').replace(/:/g, '": "') + '"}';
          // let stringified = JSON.stringify(context);
          let splitedContext = context.split(",");
          for (let i = 0; i < splitedContext.length; i++) {
              let text = splitedContext[i];
              let splitedText = text.split(":");
              let key = splitedText[0];
              let value = splitedText[1];
              let realValue = LivenHelpers.deep_value(this.target, value)
              if(realValue) {
                data[[key]] = realValue;
              } else {
                data[[key]] = value;
        ...
        let compiledElement = new DOMParser ()
          .parseFromString (temp (data), 'text/html')
          .documentElement.querySelector ('body').firstElementChild;
          compiledElement.setAttribute(":key", element.shadowElement.getAttribute(":key"));
          compiledElement.setAttribute(":context", element.shadowElement.getAttribute(":context"));
          ...
          let updated = compiledElement.cloneNode(true);
          element.updated.replaceWith(updated);
    ...
    try {
      ...
      elements.forEach (element => {
        ...
        let compiledElement = new DOMParser ()
          .parseFromString (temp (data), 'text/html')
          .documentElement.querySelector ('body').firstElementChild;
          // compiledElement.setAttribute("key-condition", element.shadowElement.getAttribute("key-condition"));
          // compiledElement.setAttribute("A-if", element.shadowElement.getAttribute("A-if"));
          // compiledElement.setAttribute("A-not", element.shadowElement.getAttribute("A-not"));
          let shadowElement = compiledElement.cloneNode (true);
          element.shadowElement.replaceWith(shadowElement);
          element.shadowElement = shadowElement;
          let updatedElem = LivenHelpers.removeAttributes(compiledElement);
